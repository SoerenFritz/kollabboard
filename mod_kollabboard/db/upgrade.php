 ...
 */
function xmldb_whiteboard_upgrade($oldversion = 0) {
function xmldb_kollabboard_upgrade($oldversion) {
    ...
    if ($oldversion < 2026061202) {
        // If the plugin was installed with the old table name, rename it now.
        $oldtable = new xmldb_table('whiteboard_instances');

        if (!$dbman->table_exists('whiteboard') && $dbman->table_exists('whiteboard_instances')) {
            $dbman->rename_table($oldtable, 'whiteboard');
        }

        upgrade_mod_savepoint(true, 2026061202, 'whiteboard');
